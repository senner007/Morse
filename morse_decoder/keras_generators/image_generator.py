...
class Image_Generator_RAW(keras.utils.Sequence) :
    ...
    def __init__(self, image_amount, set_obj, apply_noise, train_img_generate, FFT_JUMP, batch_size, image_target_size, image_prepocessors, label_func) :
        self.apply_noise = apply_noise
        self.train_image_generate = train_img_generate
    ...
    def __len__(self) :
        return (np.ceil(len(self.image_amount) / float(self.batch_size))).astype(np.int)
    ...
    def __getitem__(self, idx) :
        ...
        random_sets = [self.set_obj.get_random() for n in range(128)]
        ...
        signal_noises = [self.apply_noise(signal) for signal in signals_shiftet]
        images_noise = [self.train_img_generate(signal_noise, self.FFT_JUMP) for signal_noise in signal_noises]
